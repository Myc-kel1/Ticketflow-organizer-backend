...
class EventService:
    ...
    def create_event(event_data: EventCreate, organizer_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(event_data.start_date, 'isoformat'):
                event_dict["start_date"] = event_data.start_date.isoformat()
            if hasattr(event_data.end_date, 'isoformat'):
    ...
    def update_event(
        ...
        try:
            ...
            if "start_date" in update_dict and hasattr(update_dict["start_date"], 'isoformat'):
                update_dict["start_date"] = update_dict["start_date"].isoformat()
            if "end_date" in update_dict and hasattr(update_dict["end_date"], 'isoformat'):
    ...
    @staticmethod
    def update_event_images(event_id: str, image_urls: List[str]) -> Dict[str, Any]:
        """Update event with image URLs after upload."""
        try:
            response = (
                supabase.table("events")
            ...
            if not response.data:
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Failed to update event images"
                )
